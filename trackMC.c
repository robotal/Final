...
void trackMC(){
	...
	for(int i = 0; i < n; i++){
		...
		for(int j = 1; j <= layers; j++){
			...
			float erry1 = max(absolute(epsilon*cos(Ephimax)/cos(phi1)), absolute(epsilon/cos(phi1-Ephimax)));
			...
			float erry2 = max(absolute(epsilon*cos(Ephimax)/cos(phi2)), absolute(epsilon/cos(phi2-Ephimax)));
